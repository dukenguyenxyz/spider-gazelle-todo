...
describe TasksController, focus: true do
  ...
  it "should find the task" do
    response = context_generator("GET", "show", "/tasks/#{task1.id}", task1.id)
  ...
  it "should create and update a task" do
    ...
    response1 = context_generator("POST", "create", "/tasks", body: body1.to_json)
    ...
    response2 = context_generator("PATCH", "update", "/tasks/#{created["id"]}", resource_id: created["id"].to_s, body: body2.to_json)
  ...
  it "should not find the task" do
    response1 = context_generator("GET", "show", "/tasks/1233", 1233)
    status_code(response1).should eq(404) # Status 404
    data(response1).as_h["message"].should eq("Not Found")

    response2 = context_generator("DELETE", "destroy", "/tasks/1233", 1233)
    status_code(response2).should eq(404) # Status 404
    data(response2).as_h["message"].should eq("Not Found")

    body2 = {note: "read Nietzsche's zarathustra"}
    response3 = context_generator("PATCH", "update", "/tasks/1233", resource_id: "1233", body: body2.to_json)
